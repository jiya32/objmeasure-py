...
while True:
    ...
    img, conts = utilities.getContours(img,minArea=50000, filter=4)
    #findout the biggest contour
    if len(conts)!=0:
        biggest = conts[0][2]
        #print(biggest)
        imgWarp = utilities.warpImg(img, biggest, wP, hP)

        img2, conts2 = utilities.getContours(imgWarp, minArea=2000, filter=4, cThr=[50,50], draw= False)
